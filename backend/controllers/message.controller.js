  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        pagination: { page: parseInt(page), limit: parseInt(limit), total, hasMore: skip + messages.length < total }
      }
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
...
const editMessage = async (req, res) => {
  try {
    ...
    const message = await Message.findOne({
      _id: req.params.messageId, sender: req.user._id, type: 'text'
    });
    if (!message) {
      return res.status(404).json({ success: false, message: 'Message not found' });
    }
...
const reactToMessage = async (req, res) => {
  try {
    ...
    await message.save();
    res.status(200).json({ success: true, data: { reactions: message.reactions } });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
module.exports = { sendMessage, getMessages, deleteMessage, editMessage, reactToMessage, markAsRead };
